...
void ServoPwmDutyCompare(void)//脉宽变化比较及速度控制
{
	...
	static uint16 ServoPwmDutyIncTimes2;//Edison [2021/03/26]-[10:59:50]- 	
	...
	if(ServoPwmDutyHaveChange)//停止运动并且脉宽发生变化时才进行计算      ServoRunning == FALSE && 
	{
		...
		//Edison [2021/03/26]-[10:57:59]-Start
		ServoPwmDutyIncTimes2 = ServoPwmDutyIncTimes;
		//Edison [2021/03/26]-[10:57:59]-End
	...
	if(ServoRunning)
	{
		...
		//Edison [2021/03/22]-[17:14:50]-Start
   		if(Edison_DBG == 2) 
		{
			ShowDec(ServoPwmDutyIncTimes2-ServoPwmDutyIncTimes);
		}	
		//Edison [2021/03/22]-[17:14:50]-End		
		...
		for(i=0;i<8;i++)
		{
			...
			//Edison [2021/03/22]-[16:55:12]-Start
      		if(Edison_DBG == 2) 
			{
				if((i >= 1) && (i <= 6)) ShowHEX(ServoPwmDuty[i]);
			}
			//Edison [2021/03/22]-[16:55:12]-End			
		}	
		//Edison [2021/03/22]-[17:06:53]- 		
   		if(Edison_DBG == 2) 
		{
			if(ServoPwmDutyIncTimes == 0)	UART1SendDataPacket("\n------------------------------\n",32);
			Uart1SendData('\n');  
		}	
